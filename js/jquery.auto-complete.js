            ...
            function suggest(data){
                ...
                if (data.length && val.length >= o.minChars) {
                    ...
                    for (i = 0; i < data.length; i++)
                        //console.log(data[i]);
                        if (data[i] != null) {
                            nombre = data[i].replace(re, "<b>$1</b>");
                            cedula = data[i].match(/[0-9]+/g);
                            
                            s += '<div id="'+cedula+'" class="autocomplete-suggestion" data-val="' + data[i] + '">' + nombre + '</div>';
                            
                            
                        }
                    var repre = $('input:checkbox#representado:checked').length;
                    if(repre == 1){
                        s += '<div id="add" class="autocomplete-suggestion cursor" style="text-align: center;" data-val="'+cedula+'">Agregar Nuevo</div>';
                    }
                    that.sb.html(s).show();
                }else{
                    that.sb.hide();
