...
//**************************************  
  // Added Options
  // date 15/10/2004 author glauxbond@email.it
  // 'shadedHeadings'=> 0,1, default 0 (1-> print a background color into the table headings
  // 'shadeHeadingCols'=>(r,g,b) array, defining the colour of the backgound of headings, default is (0.9,0.9,0.9)
  ...
  $defaults = array(
    ...
    ,'shadedHeadings'=>0,'shadeHeadingCols'=>array(0.9,0.9,0.9)
    );
...
//*************************	  
//Lines added to manage background color for headings
//begin
    if ($options['shadedHeadings']){
	    $this->saveState();
...
//*******************    
//Lines addae to manage background color for headings
//begin
      if ($options['shadedHeadings']){
        $this->closeObject();
        $this->setColor($options['shadeHeadingCols'][0],$options['shadeHeadingCols'][1],$options['shadeHeadingCols'][2],1);
        $this->filledRectangle($x0-$options['gap']/2,$y,$x1-$x0,$headingHeight);
        $this->reopenObject($textHeadingsObjectId);
		$this->closeObject();
	   	$this->restoreState();
 	 }
...
//begin
		  if ($options['shadedHeadings']){
    	    ...
    	    $this->closeObject();
	        $this->setColor($options['shadeHeadingCols'][0],$options['shadeHeadingCols'][1],$options['shadeHeadingCols'][2],1);
        	$this->filledRectangle($x0-$options['gap']/2,$y-$headingHeight,$x1-$x0,$headingHeight-2);
        	$this->reopenObject($textHeadingsObjectId);
  			$this->closeObject();
	   		$this->restoreState();
 		  }
