...
$Cont["Campos"] = "NRegEve,NRegCat,Estatus";
$Cont["TitCam"] = "Evento,Categoria,Estatus";
$Cont["Orden"] = "ORDER BY Total DESC";

$TitCampos = "Nombre,Cedula,Asociacion,Puntos";
$NomCampos = "NRegAtl,Asociacion,Cedula,Total";
...
$QryPri = "
SELECT (SELECT Nombre FROM atletas WHERE NRegistro = NRegAtl) AS Nombre,
	   (SELECT Cedula FROM atletas WHERE NRegistro = NRegAtl) AS Cedula,
	   (SELECT Nombre FROM Asociaciones WHERE NRegistro IN (SELECT Asociacion FROM atletas WHERE NRegistro = NRegAtl)) AS Asociacion,
	    SUM(Puntos) AS Total 
	FROM competencia AS A
	WHERE NRegistro > 0 ".$Filtros."
	GROUP BY NRegAtl ".$Orden;
